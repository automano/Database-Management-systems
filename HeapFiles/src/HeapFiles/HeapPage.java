...
public class HeapPage {
	...
	public boolean slotOccupied(int s) {
		...
		int byteNum = s / 8;
		int offset = s % 8;
		int value = header[byteNum] & (byte) 1 << offset;
		if (value == 0) {
			return false;
		} else {
			return true;
	...
	public void addTuple(Tuple t) throws Exception {
		...
		if (!td.equals(t.getDesc())) {
			throw new Exception("Tuple and Page have different TupleDesc.");
		}
		int emptySlot = -1;
		boolean empty = false;
		for (int i=0; i<header.length; i++) {
		...
		setSlotOccupied(emptySlot, true);
		t.setPid(this.getId());
		t.setId(emptySlot);
		tuples[emptySlot] = t;
	}
	...
	public void deleteTuple(Tuple t) throws Exception {
		...
		if (t.getPid() != this.getId()) {
			throw new Exception("Tuple "+t.getPid()+" not in HeapPage "+this.getId());
		}
		
//		for (int i=0; i<this.getNumSlots(); i++) {
//			if (tuples[i].equals(t)) {
//				if (!slotOccupied(i)) {
//					throw new Exception("Tuple "+t.getPid()+" in slot "+i+" is already empty. ");
//				}
...
//		throw new Exception("Tuple "+t.getPid()+" not in HeapPage "+this.getId());
		...
		if (!slotOccupied(slot)) {
			throw new Exception("Tuple "+t.getPid()+" in slot "+slot+" is already empty. ");
		}
//		if (!tuples[slot].equals(t)) {
//			throw new Exception("Target tuple content doesn't match");
//		}
